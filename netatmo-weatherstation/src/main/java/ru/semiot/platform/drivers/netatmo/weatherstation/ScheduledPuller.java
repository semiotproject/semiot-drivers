  ...
  public void run() {
    try {
      ...
      JSONArray data = netAtmoAPI.getPublicData(
          config.getAsString(Keys.LATITUDE_NORTH_EAST),
          config.getAsString(Keys.LONGITUDE_NORTH_EAST),
          config.getAsString(Keys.LATITUDE_SOUTH_WEST),
          config.getAsString(Keys.LONGITUDE_SOUTH_WEST));
      ...
      if (data != null) {
        ...
        List<WeatherStationObservation> observations = wsFactory.parseObservations(
            data, config.getAsString(Keys.ONLY_NEW_OBS).equalsIgnoreCase("false"));
        ...
        for (WeatherStationObservation newObs : observations) {
          ...
          if (config.getAsString(Keys.ONLY_NEW_OBS).equalsIgnoreCase("false")
              || oldObs == null
              || !newObs.getProperty(DeviceProperties.OBSERVATION_TIMESTAMP)
